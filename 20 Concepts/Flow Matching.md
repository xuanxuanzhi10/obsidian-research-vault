...
把噪声样本想成被风吹动的粒子。网络不直接猜“最终应该落在哪里”，而是在任意中间位置告诉粒子此刻的风向和速度；沿这个速度场积分，粒子最终形成合理动作分布。
...
**优势：** 避免单次 MSE 的 mode averaging，直接处理连续动作块，并可结合固定条件的 KV Cache。

**代价：** 推理仍需多次网络调用；solver 步数、时间采样和速度参数化都会影响质量与延迟。

**边界：** “可表达多峰”不代表数据不足时自动发现所有合理模式，也不能把论文特定的 10 steps 当成 Flow Matching 的普遍常数。
